...
class HypeStrategy:
    """
    Attention velocity + price/volume confirms
    """
    ...
    def _regime_and_breadth(self) -> Tuple[bool, float]:
        # BTC regime + market breadth
        try:
    ...
    def scan(self, lookback_hours_news: int=24, threshold: float=70.0, vol_z_threshold: float=1.5):
        ...
        def _z(df, col):
            s=df[col]; std=s.std(ddof=1); return (s - s.mean())/(std if std else 1.0)
        news_agg["news_peer_z"] = _z(news_agg,"mentions")
        ...
        if trends_df.empty:
            trends_df = pd.DataFrame({"product_id":[a["product_id"] for a in self.assets],
                                      "trends_last":[0.0]*len(self.assets),
                                      "trends_roc":[0.0]*len(self.assets)})
        std = trends_df["trends_roc"].std(ddof=1)
        trends_df["trends_z"] = (trends_df["trends_roc"] - trends_df["trends_roc"].mean())/(std if std else 1.0)

        # Novelty via TF-IDF
        if not news_details.empty:
            docs = news_details.groupby("product_id")["title"].apply(lambda s: " . ".join(s.tolist()))
            pids = docs.index.tolist()
            if pids:
                X = TfidfVectorizer(max_features=1000, ngram_range=(1,2)).fit_transform(docs.values)
        ...
        novelty["novelty_z"] = (novelty["novelty"] - novelty["novelty"].mean())/(stdn if stdn else 1.0)
